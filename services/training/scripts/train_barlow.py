"""
Barlow Twins 训练：使用相似对、无数据增强。
损失为交叉相关矩阵：对角接近 1（不变性），非对角接近 0（去冗余）。
供 main.py 步骤4 可选调用（Config.training_method = "barlow"）。
"""
import sys
...
class BarlowTwinsModel(nn.Module):
    """
    Barlow Twins：encoder -> projector -> z。
    forward(x) 返回 z，两条分支各调用一次，损失为交叉相关矩阵偏离单位阵。
    """
...
def barlow_twins_loss(
    ...
    """
    Barlow Twins 损失：交叉相关矩阵 C 应对角为 1、非对角为 0。
    - 对每个特征维在 batch 上标准化，再算 C = (1/B) z1_norm.T @ z2_norm
    - on_diag: 对角项偏离 1 的平方的**均值**（按维数平均，避免 D 大时 loss 爆炸）
    - off_diag: 非对角项平方的**均值**
    - loss = on_diag + lambda_off_diag * off_diag，量级约在 0～几
    """
    B, D = z1.shape
    # 按特征维在 batch 上标准化（每列均值 0、方差 1）
    z1_norm = (z1 - z1.mean(dim=0)) / (z1.std(dim=0) + eps)
    z2_norm = (z2 - z2.mean(dim=0)) / (z2.std(dim=0) + eps)
    # C [D, D]
    C = (z1_norm.T @ z2_norm) / B
    on_diag = (1 - C.diag()).pow(2).mean()
    off_diag = (C.pow(2).sum() - C.diag().pow(2).sum()) / max(D * (D - 1), 1)
...
class BarlowTwinsTrainer:
    ...
    def train_epoch(self, dataloader: DataLoader, epoch: int) -> Dict[str, float]:
        ...
        for batch in pbar:
            ...
            loss = barlow_twins_loss(z1, z2, lambda_off_diag=self.lambda_off_diag)
...
def train_barlow(
    ...
    image_size: int = 224,
    log_dir: str = "services/training/logs/barlow",
    device: str = "auto",
    val_split: float = 0.2,
    lambda_off_diag: float = 0.005,
) -> Tuple[BarlowTwinsTrainer, BarlowTwinsModel]:
    """
    使用相似对、无增强的 Barlow Twins 训练。接口与 train_simsiam / main 对齐。
    """
    if device == "auto":
    ...
    print("🏗️ Building Barlow Twins (CLIP encoder + projector)...")
    encoder = CLIPEncoder(model_name=model_name, embedding_dim=embedding_dim)
    model = BarlowTwinsModel(encoder=encoder, encoder_dim=embedding_dim)
    ...
    print("✅ Barlow Twins training completed.")
    return trainer, model


if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description="Train Barlow Twins from similar-pair NPY (no aug)")
    ...
    p.add_argument("--lambda_off_diag", type=float, default=0.005)
    p.add_argument("--log_dir", type=str, default="services/training/logs/barlow")
    args = p.parse_args()
    train_barlow(
        ...
        lambda_off_diag=args.lambda_off_diag,
